let toprateanimecards = document.getElementById("toprate-anime-cards");

let maintoprateanime = AnimeMovieAndSeriesArray.filter((movie) =>{
  return movie.rating > 7.9;
})

maintoprateanime.sort((a,b) => b.rating - a.rating);

for (let i = 0; i < 8; i++) {
  const Source = maintoprateanime[i].Source;
  const Name = maintoprateanime[i].name;
  const Year = maintoprateanime[i].year;
  const Href = maintoprateanime[i].href;
  let animeType = maintoprateanime[i].animetype;
  let sub = maintoprateanime[i].sub;
  let rating = maintoprateanime[i].rating;
    ResultDiv = `
...
toprateanimecards.innerHTML += ResultDiv;
}
